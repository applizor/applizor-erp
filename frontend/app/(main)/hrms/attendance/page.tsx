...
export default function AttendancePage() {
        ...
        <div className="space-y-6">
            ...
            <div className="ent-card p-6 bg-gradient-to-br from-white to-gray-50 flex flex-col items-center justify-center border-primary-100 shadow-xl shadow-primary-900/5">
                <div className="text-center mb-6">
                    ...
                    <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 bg-white rounded-md border border-gray-100 shadow-sm">
                ...
                <div className="flex items-center space-x-4">
                    {!todayStatus?.checkedIn ? (
                        <button
                            ...
                            disabled={geoLoading}
                            className="bg-primary-900 hover:bg-black text-white px-10 py-4 rounded-md font-black text-[12px] uppercase tracking-widest shadow-2xl shadow-primary-900/40 flex items-center gap-3 transition-all active:scale-95 disabled:opacity-50"
                        >
                            <MapPin size={18} />
                        ...
                        <button
                            ...
                            className="bg-rose-600 hover:bg-rose-700 text-white px-10 py-4 rounded-md font-black text-[12px] uppercase tracking-widest shadow-2xl shadow-rose-900/30 flex items-center gap-3 transition-all active:scale-95"
                        >
                            <LogOut size={18} />
                            Terminate Shift Session
                        </button>
                    ) : (
                        <div className="px-8 py-4 bg-emerald-50 border border-emerald-100 rounded-md flex items-center gap-3">
                ...
                {todayStatus?.checkedIn && (
                    <div className="mt-6 flex items-center gap-4 text-[10px] font-bold text-gray-500 bg-gray-100/50 px-4 py-2 rounded-md">
                                ...
                                <tr key={record.id}>
                                    ...
                                    <td>
                                        <div className="flex items-center gap-2 font-bold text-gray-600 text-[10px]">
                                            <span className="px-1.5 py-0.5 bg-gray-50 rounded-md border border-gray-100">{record.checkIn ? new Date(record.checkIn).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '---'}</span>
                                            <ChevronRight size={10} className="text-gray-300" />
                                            <span className="px-1.5 py-0.5 bg-gray-50 rounded-md border border-gray-100">{record.checkOut ? new Date(record.checkOut).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'RUNNING'}</span>
                                        </div>
                                    </td>
    ...
    const renderAdminLogs = () => (
        <div className="space-y-4">
            {/* Global Filters */}
            <div className="bg-white p-4 rounded-md border border-gray-200 shadow-sm grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                ...
                <button
                    ...
                    className="flex items-center justify-center gap-2 bg-primary-900 text-white px-4 py-2.5 rounded-md font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all"
                >
                        ...
                        {adminLogs.map((log: any) => (
                            <tr key={log.id}>
                                <td className="flex items-center gap-3">
                                    <div className="w-7 h-7 rounded-md bg-gray-100 flex items-center justify-center font-black text-[10px] text-gray-600 border border-gray-200 uppercase">
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white p-5 rounded-md border border-gray-200 shadow-sm gap-4">
                ...
                <div className="flex bg-gray-100 p-1 rounded-md border border-gray-200">
                    <button
                        ...
                        className={`flex items-center gap-2 py-2 px-4 rounded-md font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'my-attendance' ? 'bg-white text-primary-900 shadow-md ring-1 ring-black/5' : 'text-gray-500 hover:text-gray-900'
                            }`}
                    >
                    ...
                    {isAdmin && (
                        <button
                            ...
                            className={`flex items-center gap-2 py-2 px-4 rounded-md font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'admin-logs' ? 'bg-white text-primary-900 shadow-md ring-1 ring-black/5' : 'text-gray-500 hover:text-gray-900'
                                }`}
                        >
            ...
            {loading && !geoLoading && (
                <div className="py-20 flex flex-col items-center justify-center bg-white/50 rounded-md border border-dashed border-gray-200 animate-pulse">
