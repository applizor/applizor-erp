...
export default function DocumentsPage() {
    ...
    const handleInstantGenerate = async (e: React.FormEvent) => {
        ...
        setGenerating(true);
        try {
            const response = await api.post('/documents/generate', instantForm, { responseType: 'blob' });
                    ...
                    <div className="bg-primary-50/50 border border-primary-100 rounded-md p-4 mb-8 flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-primary-700 leading-relaxed">
                                Use this for one-off letters. For reusable docs, switch to <strong>Templates</strong> tab.
                            </p>
                        </div>
                    </div>

                    <form onSubmit={handleInstantGenerate} className="space-y-6 max-w-4xl">
                        ...
                        <div className="ent-form-group">
                            ...
                            <textarea
                                required
                                rows={10}
                                value={instantForm.content}
                                onChange={(e) => setInstantForm({ ...instantForm, content: e.target.value })}
                                className="ent-textarea font-mono text-sm leading-relaxed"
                                placeholder="Type your letter content here..."
                            />
                        </div>
                        <div className="flex justify-end pt-4">
                            <Button type="submit" disabled={generating} className="btn-primary min-w-[200px]">
                                {generating ? <><LoadingSpinner size="sm" /> Forging...</> : 'Generate PDF'}
                            </Button>
                        </div>
                    </form>
                </div>
