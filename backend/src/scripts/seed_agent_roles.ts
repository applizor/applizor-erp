    ...
    try {
        ...
        for (const ghost of ghostCompanies) {
            if (ghost.id !== APPLIZOR_COMPANY_ID) {
                ...
                // 1. Delete template components & employee components
                try {
                    await prisma.salaryTemplateComponent.deleteMany({
                        where: {
                            OR: [
                                { template: { companyId: cid } },
                                { component: { companyId: cid } }
                            ]
                        }
                    });
                } catch (e) {}
                ...
                try {
                    await prisma.taxInvestment.deleteMany({
                        where: {
                            declaration: { employee: { companyId: cid } }
                        }
                    });
                } catch (e) {}
                ...
                // Delete Quotation and Lead dependencies
                try {
                    await prisma.quotationItemTax.deleteMany({
                        where: {
                            quotationItem: { quotation: { companyId: cid } }
                        }
                    });
                } catch (e) {}
                ...
                try {
                    await prisma.invoiceItemTax.deleteMany({
                        where: {
                            invoiceItem: { invoice: { companyId: cid } }
                        }
                    });
                } catch (e) {}
                ...
                try {
                    await prisma.taskLink.deleteMany({
                        where: {
                            OR: [
                                { sourceTask: { project: { companyId: cid } } },
                                { targetTask: { project: { companyId: cid } } }
                            ]
                        }
                    });
                } catch (e) {}
                ...
                console.log(`✅ Deleted ghost company "${ghost.name}" (${ghost.id}) and all its related records successfully!`);
            }
        }
    } catch (err) {
        console.warn('⚠️ Warning during ghost company cleanup:', err);
    }
