...
const getS3Config = async (companyId?: string) => {
    ...
    if (!accessKeyId || !secretAccessKey) {
        throw new Error('S3 storage is not configured. Missing AWS_ACCESS_KEY_ID or AWS_SECRET_ACCESS_KEY in environment.');
    }
...
export class StorageService {
    ...
    /**
     * Upload a file to S3 strictly (No local fallback allowed)
     ...
     * @returns The direct static S3 URL
     */
    static async uploadFile(fileBuffer: Buffer, fileName: string, mimeType: string, companyId?: string): Promise<string> {
        ...
        try {
            ...
            // Return the direct static S3 URL format
            return `${s3Conf.endpoint}/${s3Conf.bucket}/${prefixName}`;
        } catch (error: any) {
    ...
    /**
     * Get a direct static URL for a file
     */
    static async getFileUrl(fileKey: string, companyId?: string): Promise<string> {
        ...
        // Return direct static S3 URL format
