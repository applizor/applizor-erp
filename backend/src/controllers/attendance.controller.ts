...
export const checkIn = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (employee.company.allowedIPs) {
            ...
            if (!matches) {
                if (!(clientIp as string).includes('127.0.0.1') && !(clientIp as string).includes('::1')) {
...
export const getAllAttendance = async (req: AuthRequest, res: Response) => {
    try {
        ...
        // 2. Determine Scope
        const scope = PermissionService.getPermissionScope(req.user, 'Attendance', 'read');
        ...
        if (employeeName) {
            where.employee = {
                OR: [
                    { firstName: { contains: employeeName as string, mode: 'insensitive' } },
                    { lastName: { contains: employeeName as string, mode: 'insensitive' } }
                ]
            };
        }
        ...
        if (month && year) {
            ...
            holidays.forEach(holiday => {
                ...
                Object.values(musterRoll).forEach((row: any) => {
...
export const getMyAttendance = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (!PermissionService.hasBasicPermission(req.user, 'Attendance', 'read')) {
            // return res.status(403).json({ error: 'Access denied' });
