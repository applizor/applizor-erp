...
exports.uploadAttachment = exports.getAllBalances = exports.getMyBalances = exports.calculateLeaveDaysValue = exports.updateLeaveStatus = exports.getAllLeaveRequests = exports.getMyLeaveRequests = exports.createLeaveRequest = exports.deleteLeaveType = exports.updateLeaveType = exports.createLeaveType = exports.getLeaveTypes = void 0;
...
const getLeaveTypes = async (req, res) => {
    try {
        ...
        if (!permission_service_1.PermissionService.hasBasicPermission(req.user, 'LeaveType', 'read')) {
        ...
        const isAdmin = permission_service_1.PermissionService.hasBasicPermission(req.user, 'Leave', 'delete'); // Proxy for Admin/Manager who manages types?
        ...
        const employee = await client_1.default.employee.findUnique({ where: { userId } });
        // If no employee record (e.g. Super Admin), return all.
        if (!employee) {
            const allTypes = await client_1.default.leaveType.findMany({ orderBy: { name: 'asc' } });
...
const createLeaveType = async (req, res) => {
    try {
        ...
        if (!permission_service_1.PermissionService.hasBasicPermission(req.user, 'LeaveType', 'create')) {
        ...
        const eligibleEmployees = await client_1.default.employee.findMany({
            where: whereClause,
            select: { id: true }
        });
        if (eligibleEmployees.length > 0) {
            ...
            await client_1.default.employeeLeaveBalance.createMany({
                data: balanceData
            });
        }
...
const updateLeaveType = async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        if (!permission_service_1.PermissionService.hasBasicPermission(req.user, 'Leave', 'delete')) {
...
const deleteLeaveType = async (req, res) => {
    try {
        ...
        const existingUsage = await client_1.default.leaveRequest.count({
        ...
        await client_1.default.leaveType.delete({ where: { id } });
        await (0, audit_service_1.logAction)(req, {
            action: 'DELETE',
            module: 'LEAVE',
            entityType: 'LeaveType',
...
const createLeaveRequest = async (req, res) => {
    try {
        ...
        if (!permission_service_1.PermissionService.hasBasicPermission(req.user, 'Leave', 'create')) {
        ...
        if (employeeId && employeeId !== employee.id) {
            if (!permission_service_1.PermissionService.hasBasicPermission(req.user, 'Leave', 'update')) {
                return res.status(403).json({ error: 'Access denied: No permission to assign leave to others.' });
            }
        }
        const targetEmployee = employeeId ? await client_1.default.employee.findUnique({ where: { id: employeeId } }) : employee;
        if (!targetEmployee)
        ...
        // Fetch Leave Type
        const leaveType = await client_1.default.leaveType.findUnique({ where: { id: leaveTypeId } });
        if (!leaveType)
        ...
        const policySettings = typeof leaveType.policySettings === 'object' && leaveType.policySettings
        ...
        if (leaveType.monthlyLimit > 0) {
            ...
            const existingRequestsInMonth = await client_1.default.leaveRequest.findMany({
                where: {
                    employeeId: targetEmployee.id,
                    leaveTypeId,
                    status: { not: 'rejected' },
        ...
        const pendingReqs = await client_1.default.leaveRequest.findMany({
            where: {
                employeeId: targetEmployee.id,
                leaveTypeId,
                status: 'pending',
        ...
        if (status === 'approved') {
            ...
            if (paidDays > 0) {
                ...
                const existingBalance = await client_1.default.employeeLeaveBalance.findFirst({
                    where: {
                        employeeId: targetEmployee.id,
                        leaveTypeId: leaveTypeId,
                        year
...
const getMyLeaveRequests = async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: 'User not authenticated' });
        // Basic Access Check
        if (!permission_service_1.PermissionService.hasBasicPermission(req.user, 'Leave', 'read')) {
            return res.status(403).json({ error: 'Access denied' });
        }
        const employee = await client_1.default.employee.findUnique({ where: { userId } });
        if (!employee)
...
const getAllLeaveRequests = async (req, res) => {
    try {
        ...
        // 2. Determine Scope
        const scope = permission_service_1.PermissionService.getPermissionScope(req.user, 'Leave', 'read');
...
const updateLeaveStatus = async (req, res) => {
    try {
        ...
        const existing = await client_1.default.leaveRequest.findUnique({
            where: { id },
        ...
        // Simple verification:
        const scope = permission_service_1.PermissionService.getPermissionScope(req.user, 'Leave', 'update');
        let hasAccess = false;
        if (scope.all)
            hasAccess = true;
        else {
            ...
            const currentEmpId = currentUserEmployee?.id;
            if (scope.owned && existing.employeeId === currentEmpId)
                hasAccess = true; // Can I approve my own? Policy... maybe? Usually No.
            if (scope.added && existing.employee.createdById === userId)
                hasAccess = true;
        }
        if (!hasAccess) {
        ...
        const leaveRequest = await client_1.default.leaveRequest.update({
            where: { id },
            data: {
                status,
                approvedBy: userId,
        ...
        if (status === 'approved') {
            ...
            if (paidDays > 0) {
                ...
                if (existingBalance) {
                    ...
                    const leaveType = await client_1.default.leaveType.findUnique({
...
const calculateLeaveDaysValue = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ error: 'User not authenticated' });
        ...
        if (employee.shiftId) {
            ...
            if (shift && shift.workDays) {
                ...
                const wd = shift.workDays;
                if (Array.isArray(wd)) {
                    shiftWorkDays = wd.map((d) => String(d).toLowerCase());
                }
            }
        }
        let calculatedDays = 0;
...
exports.calculateLeaveDaysValue = calculateLeaveDaysValue;
...
const getMyBalances = async (req, res) => {
    try {
        ...
        const hasAccess = permission_service_1.PermissionService.hasBasicPermission(req.user, 'LeaveBalance', 'read');
        if (!hasAccess) {
        ...
        if (!employee)
            return res.status(404).json({ error: 'Employee record not found' });
        const rawBalances = await client_1.default.employeeLeaveBalance.findMany({
            where: {
...
const getAllBalances = async (req, res) => {
    try {
        ...
        const scope = permission_service_1.PermissionService.getPermissionScope(req.user, 'LeaveBalance', 'read');
