...
class DocumentGenerationService {
     ...
     */
    static async generateDocx(templateBuffer, data) {
        try {
            const report = await (0, docx_templates_1.default)({
    ...
    static async convertToPdf(docxBuffer) {
        try {
            ...
            const response = await axios_1.default.post(`${GOTENBERG_URL}/forms/libreoffice/convert`, form, {
     ...
     */
    static async applyLetterhead(pdfBuffer, letterheadBuffer, mode) {
        if (mode === 'NONE' || !letterheadBuffer) {
            return pdfBuffer;
        }
        try {
            const pdfDoc = await pdf_lib_1.PDFDocument.load(pdfBuffer);
            const letterheadDoc = await pdf_lib_1.PDFDocument.load(letterheadBuffer);
