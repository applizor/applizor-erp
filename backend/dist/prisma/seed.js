...
async function main() {
    ...
    const hashedPassword = await (0, password_1.hashPassword)('admin123'); // Default password
    ...
    for (const p of employeePermissions) {
        await prisma.rolePermission.upsert({
            ...
            create: {
                roleId: employeeRole.id,
                module: p.module,
                createLevel: p.create || 'none',
                readLevel: p.read || 'none',
                updateLevel: p.update || 'none',
                deleteLevel: p.delete || 'none'
            }
        });
    }
